    ...
    while i < len(L):
        ...
        if opcode == 99:
            ...
            if current_pos._replace(direction='YEE') in positions:
                ...
                color_of_current_pos = 1 if first_panel else 0
                first_panel = False
...
with open('input.txt') as f:
    ...
    outputs = amplify(L)
    # print(positions)
    ma_x, ma_y = -float('inf'), -float('inf')
    mi_x, mi_y = float('inf'), float('inf')
    for position, color in positions.items():
        ma_x, ma_y = max(ma_x, position.x), max(ma_y, position.y)
        mi_x, mi_y = min(mi_x, position.x), min(mi_y, position.y)

    ans = [[0]*(ma_x-mi_x+1) for i in range(ma_y - mi_y+1)]

    for position, color in positions.items():
        ans[position.y - mi_y][position.x - mi_x] = color
    im = Image.fromarray(np.array(ans[::-1]).astype('uint8')*255)
